...
const Card: React.FC<ITEM> = ({ id, title, poster }: ITEM) => {
  return (
    <div className="w-full rounded overflow-hidden shadow hover:opacity-90 mitem">
      <Image
        src={
      ...
      <Link href={`/movie/${id}`} style={{ width: "auto", height: "auto" }}>
        <Image src={poster} alt={title} width="220" height="270" />
      </Link>
    </div>
  );
};
